...
export class Driver {
  ...
  // 진단사 본인이 타고 가는 차량번호 — 고객에게 나가는 배정/예약변경 알림톡에
  // "진단사 차량번호"로 들어간다(개인 연락처를 노출하는 대신 이걸로 방문 차량을 알아보게 함).
  // "평가 차량"이라고 부르면 평가 대상인 고객 차로 읽혀서 라벨을 이렇게 통일했다.
  // 평가사가 앱 "내 스케줄"에서 직접 등록하고, 관리자도 대시보드에서 대신 넣을 수 있다.
  @Column({ type: 'varchar', nullable: true })
